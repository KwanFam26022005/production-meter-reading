  ...
  const mapMeters = useMemo<MapMeterItem[]>(() => {
    if (overviewData?.meters && overviewData.meters.length > 0) {
      return overviewData.meters.map((bm) => {
        ...
        return {
          ...
          utilityType: bm.utility_type,
          isActive: bm.is_active,
          lifecycleStatus: bm.lifecycle_status || (bm.is_active ? 'ACTIVE' : 'INACTIVE'),
          retiredAt: bm.retired_at || null,
          retiredBy: bm.retired_by || null,
          retirementReason: bm.retirement_reason || null,
          zoneId: bm.zone_id || zoneCfg.id,
          zoneCode: bm.zone_code || zoneCfg.code,
          zoneName: bm.zone_name || zoneCfg.name,
          presentationZoneId: bm.presentation_zone_id,
          presentationZoneName: bm.presentation_zone_name,
    ...
    return metersData.meters.map((rawMeter) => {
      ...
      return {
        ...
        lifecycleStatus: rawMeter.lifecycle_status || (rawMeter.is_active ? 'ACTIVE' : 'INACTIVE'),
        retiredAt: rawMeter.retired_at || null,
        retiredBy: rawMeter.retired_by || null,
        retirementReason: rawMeter.retirement_reason || null,
